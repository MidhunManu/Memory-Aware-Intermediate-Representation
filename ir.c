...
static void ir_emit(IrContext* ctx, IrOpcode op, IrOperand dest, IrOperand src1, IrOperand src2, ReuseHint hint) {
    ...
    inst->src1 = src1;
    inst->src2 = src2;
    inst->access_pattern = ACCESS_SEQUENTIAL;
    inst->reuse_hint = hint;
}

static IrOperand make_empty() {
    IrOperand o; o.type = OP_TYPE_EMPTY; return o;
}
static IrOperand make_temp(int idx) {
    IrOperand o; o.type = OP_TYPE_TEMP; o.index_or_val = idx; return o;
}
static IrOperand make_var(const char* name) {
    IrOperand o; o.type = OP_TYPE_VAR; strcpy(o.var_name, name); return o;
}
static IrOperand make_const(int val) {
    IrOperand o; o.type = OP_TYPE_CONST; o.index_or_val = val; return o;
}
static IrOperand make_label(int idx) {
    IrOperand o; o.type = OP_TYPE_LABEL; o.index_or_val = idx; return o;
}
...
static IrOperand ir_generate_expr(IrContext* ctx, AstNode* node) {
    if (node->type == AST_INT_LIT) {
        ...
        ir_emit(ctx, IR_OP_CONST, dest, src1, make_empty(), REUSE_LOW);
        return dest;
    } else if (node->type == AST_IDENTIFIER) {
        IrOperand dest = make_temp(ctx->next_temp++);
        IrOperand src1 = make_var(node->var_name);
        // Loading variable: dest = var
        ir_emit(ctx, IR_OP_ASSIGN, dest, src1, make_empty(), REUSE_HIGH);
        return dest;
        ...
        ir_emit(ctx, op, dest, t1, t2, REUSE_LOW);
        return dest;
    }
    return make_empty();
}
...
void ir_generate(IrContext* ctx, AstNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_VAR_DECL: {
            ...
            ir_emit(ctx, IR_OP_ASSIGN, var, val, make_empty(), REUSE_HIGH);
            break;
        }
            
        case AST_EXIT_STMT: {
            IrOperand val = ir_generate_expr(ctx, node->left);
            ir_emit(ctx, IR_OP_EXIT, make_empty(), val, make_empty(), REUSE_LOW);
            break;
        }
        ...
        case AST_IF_STMT: {
            ...
            ir_emit(ctx, IR_OP_JMP_IF_FALSE, make_empty(), cond, make_label(false_label), REUSE_LOW);
            
            ir_generate(ctx, node->true_branch);
            if (node->false_branch) {
                ir_emit(ctx, IR_OP_JMP, make_empty(), make_label(end_label), make_empty(), REUSE_LOW);
            }
            
            ir_emit(ctx, IR_OP_LABEL, make_empty(), make_label(false_label), make_empty(), REUSE_LOW);
            
            if (node->false_branch) {
                ir_generate(ctx, node->false_branch);
                ir_emit(ctx, IR_OP_LABEL, make_empty(), make_label(end_label), make_empty(), REUSE_LOW);
            }
            break;
        }
